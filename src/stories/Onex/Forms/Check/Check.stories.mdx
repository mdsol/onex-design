...
export const CheckboxesIndeterminate = () => {
  ...
  const [checkItems, setCheckItems] = React.useState({[ids.DS]: false, [ids.JS]: false, [ids.CF]: false});
  const checkboxData = React.useMemo(() => [
    {
      label: 'Design System',
      id: [ids.DS]
...
<Canvas>
  <Story name="Controlled checkbox indeterminate">
    {CheckboxesIndeterminate.bind({})}
  </Story>
</Canvas>
