...
<Meta
  ...
  argTypes={{
    ...
    placeholder: {
      description:
        'The short hint displayed in the text field before the user enters a value. By default equal to ```dateDisplayFormat```',
      table: {
        defaultValue: {
          summary: 'yyyy/MM/dd',
        },
    ...
    onChange: {
      description:
        'Callback fired when the value is changed. Returns object with two values; isValid (bool) and value (date)',
    },
    calendarClassName: {
    ...
    locale: {
      description:
        'You can view full list from https://github.com/hypeserver/react-date-range/blob/next/src/locale/index.js. Locales directly exported from date-fns/locales.',
      table: {
        defaultValue: {
