...
<Meta
  ...
  argTypes={{
    ...
    draggable: {
      description:
        'Enable drag and drop table rows. Disabled by default. If not present, must use getRowId function and pass a unique key identifier from ```data```',
      control: 'boolean',
    },
    handleDragged: {
      description: 'Callback fired when rows dragged. Returns array of row objects',
    },
    handleSort: {
      description: 'Callback fired when column sorted. Returns array of row objects',
    },
    handlePagination: {
      description: 'Callback fired when page changed. ```handlePagination: Function(pageIndex, pageSize, pageCount, pageData)```',
    }
  }}
  component={DataGrid}
/>
...
Important: react-table (now tanstack-table) is currently v8; it is important to use the [documentation for v7](https://react-table-v7.tanstack.com/docs/overview).
...
<Canvas>
  <Story
    ...
    args={{
      ...
      handleSelection: () => {},
      getRowId: (row, relativeIndex, parent) => row.studyId,
    }}
  >
    {Template.bind({})}
  </Story>
...
To add filter you have to use prop `dataGridControlProps.filters`.
This prop represented an array of table filters.

| Name          | Type    | Description                                         |
|---------------|---------|-----------------------------------------------------|
| id            | String  | Id of column.                                       |
| name          | String  | Name(Label) of filter                               |
| defaultFilter | Boolean | If this parameter is passed filter shows by default |

For representing filters used component Filter Select, for options Filter Select please check: [FilterSelect Props](/docs/onex-components-filterselect--demo).
...
Also in case if you're using ```custom``` type of column you must pass custom function ```filter``` in column.
```
...
Also, you can use callback ```dataGridControlProps.handleFilter``` - which returns filteredRows and ```dataGridControlProps.getActiveFilters``` - function for get current value of selected filters.
...
<Canvas>
  <Story
    ...
    height="28rem"
    args={{
      dataTestId: 'data-grid-filtering-test-id',
      className: 'data-grid-filtering-custom',
      dataGridControlProps: {
        secondaryActions: [
          {
            title: 'action1',
...
| type        | One of values: `'action', 'custom', 'editable', 'date-editable'`           | `'action'` can used for header and body cells, in case if you used `Button`, `Dropdown` etc.. `custom` can used for body cell, in case if you used custom table cell. `editable` and `date-editable` can used for editable body cell. |          |
